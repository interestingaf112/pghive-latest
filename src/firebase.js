  ...
  if (isFirebaseOperational()) {
    try {
      ...
      try {
        ...
        if (!response.ok) {
          // If the server failed specifically because the Admin SDK is not initialized, we fall back to client-side Firestore writes.
          if (data.error && data.error.includes('Firebase Admin SDK is not initialized')) {
            console.warn("Vercel Serverless Function has missing Firebase Admin Key configuration. Falling back to client-side Firestore transactions.");
            useFallback = true;
          } else {
        ...
        console.warn("Serverless API call failed, trying client-side fallback:", fetchErr);
        useFallback = true;
      }
      ...
      // ── Client-side Fallback Firestore Logic (retains full rules propagation retry wrappers) ──
      ...
      const usageEntry = {
        ...
        creditsSpent: 1,
        timestamp: Date.now(),
        description: 'Unlocked PG contact details (Client Fallback)',
        deviceFingerprint: currentFingerprint,
